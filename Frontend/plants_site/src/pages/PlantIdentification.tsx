...
const PlantIdentification: React.FC = () => {
  ...
  const handleIdentify = async () => {
    ...
    try {
      ...
      else setError(t('home') === 'Home' ? 'Could not identify the plant. Please try another image.' : 'نمی‌توان گیاه را شناسایی کرد. لطفاً تصویر دیگری امتحان کنید.');
    } catch (err) {
      setError(t('home') === 'Home' ? 'An error occurred while identifying the plant. Please try again.' : 'هنگام شناسایی گیاه خطایی رخ داد. لطفاً دوباره امتحان کنید.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="lg:w-3/5 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {[
                        { icon: '💧', label: isEn ? 'Watering' : 'آبیاری', value: result.watering_frequency },
                        { icon: '☀️', label: isEn ? 'Light' : 'نور', value: result.light_requirements },
                        { icon: '🌱', label: isEn ? 'Fertilizer' : 'کود', value: result.fertilizer_schedule },
                        { icon: '🌡️', label: isEn ? 'Temperature' : 'دما', value: result.temperature_range },
                        { icon: '💧', label: isEn ? 'Humidity' : 'رطوبت', value: result.humidity_level },
                        { icon: '🌍', label: isEn ? 'Soil Type' : 'نوع خاک', value: result.soil_type },
                      ].map((item, i) => (
