...
from .serializers import RegisterInitPhoneSerializer, RegisterInitEmailSerializer, RegisterVerifySerializer
from .serializers import UserSerializer, ProfilePictureSerializer
from .services import SmsIrService, send_otp_email, generate_otp_code
...
class RequestOTPView(APIView):
    ...
    def post(self, request):
        ...
        try:
            response = sms_service.send_verification_code(phone_number, settings.SMS_IR_OTP_TEMPLATE_ID, parameters)
...
class RegisterRequestOTPView(APIView):
    ...
    def post(self, request):
        ...
        if method == 'phone':
            sms = SmsIrService()
            try:
                sms.send_verification_code(identifier, settings.SMS_IR_OTP_TEMPLATE_ID,
...
class RegisterVerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        # بازیابی اطلاعات موقت از سشن
        temp_data = request.session.get('temp_reg_data')
        if not temp_data or temp_data['identifier'] != identifier:
